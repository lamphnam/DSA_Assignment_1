...
public:
    ...
    private:
        DataLoader& data_loader; // Đổi từ con trỏ thành tham chiếu
        int batch_index;
    };
private:
...
public:
    ...
    Batch<DType, LType> get_batch(int batch_index) {
        ...
        // drop_last == false -> gộp batch cuối    
        if (!drop_last && batch_index == total_batches - 1) {
            end_index = dataset_size; // Gộp vào batch cuối
        }

        int current_batch_size = end_index - start_index;

        // Lấy kích thước từ dataset
        auto batch_data_shape = this->ptr_dataset->get_data_shape();
        batch_data_shape[0] = current_batch_size; // Cập nhật kích thước batch data
        auto batch_label_shape = this->ptr_dataset->get_label_shape();
        batch_label_shape[0] = current_batch_size; // Cập nhật kích thước batch label

        // Tạo xarray cho batch data và label
        xt::xarray<DType> batch_data = xt::empty<DType>(batch_data_shape);
        xt::xarray<LType> batch_label = xt::empty<LType>(batch_label_shape);

        // Lưu data và label vào batch
        for (int i = start_index; i < end_index; ++i) {
            int index = indices[i]; // Sử dụng chỉ số từ danh sách indices

            DataLabel<DType, LType> data_label = ptr_dataset->getitem(index);

            // Chuyển đổi dữ liệu về một chiều
            auto data = data_label.getData(); // Lấy dữ liệu
            auto label = data_label.getLabel(); // Lấy nhãn

            // Sử dụng xt::view để lưu vào batch data và batch label
            xt::view(batch_data, i - start_index) = data; // Lưu dữ liệu
            xt::view(batch_label, i - start_index) = label; // Lưu nhãn
        }
